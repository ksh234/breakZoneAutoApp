...
### D-013 보강 (2026-09-22, 사용자) — 추매 기준 유지 + 저점 반등 공통 적용
- **배경:** 첫 실매매(더코디 224060): 13:45 신규 후 15분 급락(-14.6%) 중 평단 -7% 물타기가 3회 연쇄. 사용자 "이게 맞나?" → 규칙상 정상(평단 기준은 추매마다 내려감) + 별개 버그(첫 매수 상태 유실로 예산 상한 초과) 발견.
- **결정(사용자, 1차):** 평단 기준 유지 + 저가 반등 조건 추매 적용. → **같은 날 번복(사용자): 추매 기준을 직전 매수가 대비로 변경.** 평단 기준은 추매마다 기준선이 내려가 급락 시 연쇄 추매 위험. `PositionState.last_buy_price` 기록·영속화(마이그레이션 0007), 미기록 시 평단 폴백. 추매 간격 제한은 미채택.
