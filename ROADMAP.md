...
| 2 | 키움 Broker Adapter(모의) | ✅ 완료 (왕복주문 실증) |
| 3 | Supabase 스키마·중계 | ✅ 완료 (라이브 검증) |
| 4 | 전략 엔진(규칙·리스크·루프) | ✅ 완료 (단계 A 제어 실증. 첫 매매 관찰 대기) |
| 5 | Flutter 앱(모니터링·제어·설정) | ✅ 완료 (**폰 APK 설치·로그인 정상**, 5-B FCM 보류) |
| 6 | 백테스트·모의 검증 | ⬜ 다음 |
| 7 | 실계좌 전환 | ⬜ 신중히 |
...
**완료 기준**
- [x] 접근토큰 자동 발급 동작 (스모크·통합 테스트)
- [x] 임의 종목 현재가 조회 정확 (005930 실측)
- [x] 모의 매수 주문 → 주문번호 수신 → 미체결 조회 → 취소 확인 (2026-09-03 실증, ord_no 0074195)
- [x] `get_positions()`·`get_balance()` 가 키움 모의 잔고와 일치 (예탁 5천만 확인)
- [~] WebSocket 실시간 체결가 수신 — 코드 구현 완료, 장중 라이브 수신은 단계 B에서
- [x] 전략 코드가 키움 세부를 몰라도 되게 캡슐화 (BrokerAdapter)
...
- [x] 이중 실행 방지 락 동작 — `bot_lock` + LockKeeper, 원격 스모크 실측(2026-09-03)
