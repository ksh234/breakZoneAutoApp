...
| **코드 상태** | engine analysis/broker/relay/strategy + 테스트 **181개**. Flutter 앱 7화면 + **안드로이드 APK 폰 설치·로그인 정상**(설정반영 수정본 재설치 필요). 사용법 문서([사용법.md](사용법.md)). |
...
**✅ 첫 모의매매 완료(2026-09-22 더코디 신규+추매3). 이제 = 보유 종목 청산 관찰 + 운용 지속.**
- 보유: 더코디 2,779주 평단 4,307. 분할매수 4/4 소진(추매 없음). 청산: env 상단 & +15% → 50% → 이후 상한가/1차매도가+15%/고점-5%.
- 봇 재시작 시 `strategy_state` 224060 이 4회/11,991,200 인지 확인(구봇이 덮었으면 다시 보정).

**🙋 단계 B 절차 (평일 장중 09:00~15:30, 집 PC = LIVE 봇)**
...
.\.venv\Scripts\python.exe -m pytest -q                     # 테스트 181개
...
### 세션 2026-09-23 (앱 대시보드 "오늘 주문" 버그 + 잔고 항목 영웅문 정합)
- **잔고 불일치(사용자 지적):** 앱 "평가금"=총자산(추정예탁자산 49,982,188)이었고 영웅문 평가금액은 주식평가(12,060,860). "주문가능현금"은 총자산-주식평가 근사(37,921,328) → 키움 주문가능금액 37,987,660 과 66,332 차이. 평가손익(-17,812) 미표시, 당일손익은 실현만. **수정:** `get_balance` 가 kt00001(예수금상세, qry_tp=3) 추가 호출 → `deposit`(entr)·`cash`=주문가능금액(ord_alow_amt), kt00018 `tot_evlt_pl` → `unrealized_pnl`(signed). 실패 시 근사 폴백. 하트비트에 stock_value·deposit·unrealized_pnl 추가(마이그레이션 **0008** CLI 적용). 대시보드 = 총자산/주식 평가금/예수금/주문가능금액/평가손익/당일 실현손익. 리스크 가드의 현금 검사도 이제 키움 주문가능금액 기준. 테스트 +3 → **181개**. docs/01(kt00001 실측)·02·사용법. **봇 재시작 + APK 재설치 필요.**
- 사용자: 어제 주문 4건이 오늘도 "오늘 주문 4"로 표시. 원인: 대시보드가 주문 스트림(최근 50건, 날짜 무관)의 길이를 그대로 표시. 수정: `createdAt` 이 로컬 오늘 날짜인 건만 카운트. APK 재빌드(폰 재설치). 봇 코드 변경 없음(재시작 불필요).

### 세션 2026-09-22 (🎉 첫 실매매 → 연쇄 추매 분석 → 상태 유실 버그 수정 + 추매 반등 조건)
- **운용 이력(9/4~9/22):** 집 PC bat 로 장중 구동 반복(9/7·11·15·16·17·18·21·22). 9/4 통신 안정화 후 `disconnected` 재발 없음. 사용자 설정 변경: `min_price`=1300, `post_sell_gain_pct`=15, `max_unrealized_loss_krw`=500만, `add_on_drop_pct`=0.07, `entry_rebound_pct` 0.02→0.01(9/22 13:45).
- **첫 실매매(더코디 224060, 9/22):** 13:45 신규 628주@4,770 → 13:57 추매 684@4,385 → 14:00:18 추매 731@4,100 → 14:00:27 추매 736@4,075. 총 2,779주, 평단 4,307, 투입 **11,991,200원**. 사용자 "최대횟수까지 다 샀는데 맞나?" → 분석.
